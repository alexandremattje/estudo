...
## Montar imagem da aplicação
Executar na raiz do projeto ```docker build . -t beach-app```

## Para subir o container da aplicação e o container do MYSQL
Executar na raiz do project ```docker-compose up -d ```

### Se a aplicação subiu corretamente esta [url](http://localhost:8080/orders/list?productType=SURFBOARD&page=0&offSet=10) deve retornar um json e o status 200
